...
- 併せて `127.0.0.1` の閲覧専用ポートでも待ち受ける(リモート閲覧。§7・ADR-0004)
...
サイドバーのタグ一覧は入力欄でタグ自体を探せる(部分一致・大文字小文字無視)。タグを見つけるための補助であり、これ自体はアセットを絞らない。
...
- **トリガーは手動のみ**(アプリは自動生成しない。定期実行が欲しければ外部ツールから API を叩けば足りる)
  - アセット単位の「生成」ボタン
...
手元のアプリを動かしたまま、別の PC からライブラリを**読み取り専用**で見られるようにする(ADR-0004)。

- **経路**: アプリは閲覧専用ポート(`127.0.0.1:8766`)でも待ち受ける。ネットワーク越しの代理(`tailscale serve` 等)の転送先をここに向ける。バイナリ自身は `127.0.0.1` の外にバインドしない
- **本人確認**: アプリは認証を持たない。ネットワーク層(デバイス認証)で済んでいる前提とする
- **操作範囲**: 閲覧専用ポートに届いた要求は **GET と HEAD だけ**を通し、他はすべて 403。タグ編集・生成・スキャン・新規作成・キャッシュ削除・Blender起動・「Finderで表示」はいずれも不可
- **UI**: リモート閲覧では操作要素を**非表示**にする(グレーアウトではない)。判定は `GET /api/config` の `remoteViewing`
- 見られるもの: 一覧・検索・詳細・3Dプレビュー・スプライト・ファイルビューア・`.blend` を含むアセット内ファイルのダウンロード
- リモート閲覧中も表示対象は手元のライブラリそのもの。閲覧するには手元の PC が起動している必要がある
...
|インターネット接続|不要|
|オフライン動作|必須|
|ネットワーク|127.0.0.1 のみバインド。LANに露出しない(ネットワーク越しに見せる場合も、外部の代理が loopback の閲覧専用ポートへ転送する)|
|DB|SQLite|
|キャッシュ|再生成可能|
