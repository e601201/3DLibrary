...
- 孤児キャッシュも併せて掃除する。消えたカテゴリは `cache/*/{カテゴリ}/` をディレクトリごと、消えたアセットは `cache/*/{カテゴリ}/{タイトル}` のファイルを削除する
  - 生死の判定はスキャンと同じ規則(source 直下の可視ディレクトリ)による。`model.blend` を失った不完全アセットはディレクトリが残る限りアセットなので、キャッシュも残す
